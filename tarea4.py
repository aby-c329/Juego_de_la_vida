## TAREA 4 implementacio de numba para comparar con el codigo anterior (sin numba) -- instruccines en readme
...
class GameOfLifeNumba:
    ...
    # Aplicamos Numba a las funciones
    @staticmethod
    @jit(nopython=True, cache=True)
    def _count_neighbors_numba(gameState, nxC, nyC, x, y):
        tot = 0
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                if dx or dy:
                    tot += gameState[(x + dx) % nxC, (y + dy) % nyC]
        return tot

    @staticmethod
    @jit(nopython=True, cache=True)
    def _update_state_numba(gameState, new, nxC, nyC):
        for x in range(nxC):
            for y in range(nyC):
                n = GameOfLifeNumba._count_neighbors_numba(gameState, nxC, nyC, x, y)
                if gameState[x, y] == 0 and n == 3:
                    new[x, y] = 1
                elif gameState[x, y] == 1 and (n < 2 or n > 3):
                    new[x, y] = 0

    def step(self):
        new = self.gameState.copy()
        self._update_state_numba(self.gameState, new, self.nxC, self.nyC)
        self.gameState[:] = new
...
def compare_numba(nx, ny, steps):
    print("--- Comparando rendimiento con y sin Numba ---")
    
    # Sin Numba
    start_time_no_numba = time.time()
    g = GameOfLife(nxC=nx, nyC=ny)
    for _ in range(steps):
        g.step()
    end_time_no_numba = time.time()
    time_no_numba = end_time_no_numba - start_time_no_numba
    
    # Con Numba
    start_time_numba = time.time()
    gn = GameOfLifeNumba(nxC=nx, nyC=ny)
    for _ in range(steps):
        gn.step()
    end_time_numba = time.time()
    time_numba = end_time_numba - start_time_numba

    # Escribir resultados en Performance.md
    with open('Performance.md', 'w') as f:
        f.write("# Análisis de Rendimiento: Juego de la Vida\n\n")
        f.write("## Comparación de rendimiento: Versión Original vs. Numba\n\n")
        f.write(f"Tamaño de la cuadrícula: {nx}x{ny}\n")
        f.write(f"Número de pasos de simulación: {steps}\n\n")
        f.write("| Versión | Tiempo de Ejecución (s) | Aceleración (Speedup) |\n")
        f.write("|---------|-------------------------|-----------------------|\n")
        f.write(f"| Original | `{time_no_numba:.4f}` | `1.0` |\n")
        f.write(f"| Numba | `{time_numba:.4f}` | `{(time_no_numba / time_numba):.2f}` |\n")

    print(f"Comparación completada. Resultados guardados en 'Performance.md'.")
    print(f"Tiempo sin Numba: {time_no_numba:.4f}s")
    print(f"Tiempo con Numba: {time_numba:.4f}s")
    print(f"Aceleración (Speedup): {(time_no_numba / time_numba):.2f}x")
...
if __name__ == '__main__':
    ...
    sp.add_parser('plot')
    # comparar con numba
    p_numba = sp.add_parser('compare-numba')
    p_numba.add_argument('--nx', type=int, default=512)
    p_numba.add_argument('--ny', type=int, default=512)
    p_numba.add_argument('--steps', type=int, default=100)
    ...
    if args.cmd == 'profile':
        ...
        plot_scaling()
    elif args.cmd == 'compare-numba':
        compare_numba(args.nx, args.ny, args.steps)
    else:
        p.print_help()
